...
// define the item
contract MusicsharesMarketContract {
    ...
    struct AuctionItem {
        ...
        address payable collabArtistA;
        address payable collabArtistB;
    ...
    // 2 functions one for adding and the other for buying the item

    // make shure only the owner of the item can put it up for sale
    ...
    function buyitem(uint256 id) payable external ItemExists(id) IsForSale(id) HasTransferApproval(itemsForSale[id].tokenAddress, itemsForSale[id].tokenId) {
        ...
        require(msg.sender != itemsForSale[id].seller); //check the buyer is no the same as the seller
