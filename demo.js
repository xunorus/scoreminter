        ...
        hide() {
            ...
            TweenMax.to(this.DOM.reveal, .5, {
                ...
                onComplete: () => {
                    ...
                    console.log('oculta close button');
                    // window.location.pathname = "/index.html";//if not loged return to home page
            ...
            Array.from(this.DOM.el.querySelectorAll('a.grid__item')).forEach((item) => {
                ...
                if ( !item.classList.contains('grid__item--noclick') ) {
                    itemObj.DOM.el.addEventListener('click', (ev) => {
                        ev.preventDefault();
                        this.openItem(document.querySelector(item.getAttribute('href')));// aca lo selecciona
                        console.log(this.getAttribute('href'));
                        // window.history.pushState( {} , '', '?nftId=standar' );//Clearing URL hash
                        window.history.pushState( {} , '', '?nftId='+itemObj.DOM.el.id );//Clearing URL hash
  
                        // header hide
                    // header.style.opacity = 0;
                    // head.style.display= 'none';

                        console.log('GRID #276 en demo.js',itemObj.DOM.el.href);
                        console.log('GRID #276 en demo.js: itemObj ',itemObj.DOM.el.id);
                    });
                }
            });
...
console.log("iniciar has benn executed");
